...
/* PAGE LOADER
  ...
  */
pageLoader:function(interval){interval=interval||1500;pineapple.pageLoaderInput=setTimeout(pineapple.showPage,interval);return pineapple},showPage:function(){$("#pa-loader").css("display","none");$("#pa-loader-div").css("display","block");return pineapple},
/* COUNTDOWN TIMER
      Source: https://www.w3schools.com/howto/howto_js_countdown.asp
      Syntax: <script>pineapple.countdown.init('2018-12-15', 'timer', 'Timer has expired');</script>
  */
countdown:{init:function(timestamp,elementId,message){
...
const days=Math.floor(timeDifference/(1e3*60*60*24));const hours=Math.floor(timeDifference%(1e3*60*60*24)/(1e3*60*60));const minutes=Math.floor(timeDifference%(1e3*60*60)/(1e3*60));const seconds=Math.floor(timeDifference%(1e3*60)/1e3);
// Display the result in the element with id='demo'
document.getElementById(elementId).innerHTML=""+days+" days,&nbsp; "+hours+" hours,&nbsp; "+minutes+" minutes,&nbsp; "+seconds+" seconds ";
// If the count down is finished, write some text
if(timeDifference<0){clearInterval(x);document.getElementById(elementId).innerHTML=message}},1e3);return pineapple.countdown}}};$(document).ready(function(){
/* SMOOTH SCROLLER
...
$("html, body").animate({scrollTop:$(hash).offset().top-pineapple.scrollOffset},900,function(){
// Don't jump to the anchor point after scrolling to the offset
if(history.pushState){history.pushState(null,null,hash)}else{
// Add hash (#) to URL when done scrolling (default click behavior)
window.location.hash=hash}})}})
/*
  SLIDEANIM EFFECT
      Source: https://www.w3schools.com/bootstrap/bootstrap_theme_company.asp
  */;$(window).scroll(function(){$(".pa-slideanim").each(function(){const pos=$(this).offset().top;const h=window.innerHeight;const winTop=$(window).scrollTop();if(pos<winTop+h-40){$(this).addClass("pa-slide")}})})
/*
  ...
  */;$(window).scroll(function(){if($(this).scrollTop()>pineapple.navFadeValue){$(".pa-nav-fade").addClass("opaque");$(".pa-nav-fade a").addClass("opaque")}else{$(".pa-nav-fade").removeClass("opaque");$(".pa-nav-fade a").removeClass("opaque")}})});
